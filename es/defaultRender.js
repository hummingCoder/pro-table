...
var defaultRenderText = function defaultRenderText(text, valueType, index, item, columnEmptyText) {
  ...
  if (valueType === 'dateRange' && text && Array.isArray(text) && text.length === 2) {
    ...
    return React.createElement(
      'div',
      null,
      React.createElement('div', null, startText ? moment(startText).format('YYYY-MM-DD') : '-'),
      React.createElement('div', null, endText ? moment(endText).format('YYYY-MM-DD') : '-'),
    );
  }
  /**
   *如果是日期加时间类型的值
  ...
  if (valueType === 'dateTimeRange' && text && Array.isArray(text) && text.length === 2) {
    ...
    return React.createElement(
      ...
      React.createElement(
        'div',
        null,
        _startText ? moment(_startText).format('YYYY-MM-DD HH:mm:ss') : '-',
      ),
      React.createElement(
        'div',
        null,
        _endText ? moment(_endText).format('YYYY-MM-DD HH:mm:ss') : '-',
      ),
    );
  }
  /**
